...
public:
    ...
    void write_on_leader(uint32_t num_entries, bool wait_for_commit = true, shared< TestReplicatedDB > db = nullptr) {
        auto batch_size = wait_for_commit ? g_helper->runner().qdepth_ : num_entries;
        do {
        ...
        LOGINFO("my_uuid={}, {} entries are expected to be written,  I wrote {}, committed {}, rollback {}",
                boost::uuids::to_string(g_helper->my_replica_id(), num_entries, written_entries_, total_committed_cnt(),
                                        total_rollback_cnt()));
    }
